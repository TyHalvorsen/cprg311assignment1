...
public class SortingAlgorithms {
	...
	// divides - not done
	public Shape[] mergeSort(Shape[] array) { // devides array over and over till its an array of one element
		...
		Shape[] temArray1 = new Shape[array.length / 2];
		Shape[] temArray2 = new Shape[array.length / 2 + 1];

		temArray1 = mergeSort(temArray1);
		temArray2 = mergeSort(temArray2);

		return merge(temArray1, temArray2);

		// cut array into 2
		// devide it again untill (using reccursion)you get single arrays then call
		// merge to
		// merge those arrays
	...
	// combines - not done
	public Shape[] merge(Shape[] array1, Shape[] array2) {

		// takes 2 divided arrays and compares the first element of each one and add the
		// smallest first then largest
		Shape[] finalArray = null;

		while (array1.length >= 1 && array2.length >= 1) {

			if (array1[0].compareTo(array2[0]) >= 1) {

				finalArray[finalArray.length] = array2[0];
